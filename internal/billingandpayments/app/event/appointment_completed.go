...
func (h AppointmentCompletedHandler) Handle(ctx context.Context, event AppointmentCompleted) error {
	ctx, span := telemetree.AddSpan(ctx, "billingandpayments.app.event.appointment_completed.handle")
	defer span.End()
	...
	err = h.publisher.EmitEventMessage("billingandpayments.appointment_completed", msg)
	if err != nil {
		telemetree.RecordError(ctx, err)
