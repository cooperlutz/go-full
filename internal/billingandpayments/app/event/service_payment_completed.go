...
func (h ServicePaymentCompletedHandler) Handle(ctx context.Context, event ServicePaymentCompleted) error {
	ctx, span := telemetree.AddSpan(ctx, "billingandpayments.app.event.service_payment_completed.handle")
	defer span.End()
	...
	err = h.publisher.EmitEventMessage("billingandpayments.service_payment_completed", msg)
	if err != nil {
		telemetree.RecordError(ctx, err)
