...
func (h AppointmentReminderScheduledHandler) Handle(ctx context.Context, event AppointmentReminderScheduled) error {
	ctx, span := telemetree.AddSpan(ctx, "notificationsandcommunications.app.event.appointment_reminder_scheduled.handle")
	defer span.End()
	...
	err = h.publisher.EmitEventMessage("notificationsandcommunications.appointment_reminder_scheduled", msg)
	if err != nil {
		telemetree.RecordError(ctx, err)
