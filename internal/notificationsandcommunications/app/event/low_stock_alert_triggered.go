...
func (h LowStockAlertTriggeredHandler) Handle(ctx context.Context, event LowStockAlertTriggered) error {
	ctx, span := telemetree.AddSpan(ctx, "notificationsandcommunications.app.event.low_stock_alert_triggered.handle")
	defer span.End()
	...
	err = h.publisher.EmitEventMessage("notificationsandcommunications.low_stock_alert_triggered", msg)
	if err != nil {
		telemetree.RecordError(ctx, err)
