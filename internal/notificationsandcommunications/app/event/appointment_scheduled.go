...
func (h AppointmentScheduledHandler) Handle(ctx context.Context, event AppointmentScheduled) error {
	ctx, span := telemetree.AddSpan(ctx, "notificationsandcommunications.app.event.appointment_scheduled.handle")
	defer span.End()
	...
	err = h.publisher.EmitEventMessage("notificationsandcommunications.appointment_scheduled", msg)
	if err != nil {
		telemetree.RecordError(ctx, err)
