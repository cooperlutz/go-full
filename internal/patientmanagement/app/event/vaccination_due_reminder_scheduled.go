...
func (h VaccinationDueReminderScheduledHandler) Handle(ctx context.Context, event VaccinationDueReminderScheduled) error {
	ctx, span := telemetree.AddSpan(ctx, "patientmanagement.app.event.vaccination_due_reminder_scheduled.handle")
	defer span.End()
	...
	err = h.publisher.EmitEventMessage("patientmanagement.vaccination_due_reminder_scheduled", msg)
	if err != nil {
		telemetree.RecordError(ctx, err)
