...
func (h AppointmentCompletedHandler) Handle(ctx context.Context, event AppointmentCompleted) error {
	ctx, span := telemetree.AddSpan(ctx, "patientmanagement.app.event.appointment_completed.handle")
	defer span.End()
	...
	err = h.publisher.EmitEventMessage("patientmanagement.appointment_completed", msg)
	if err != nil {
		telemetree.RecordError(ctx, err)
