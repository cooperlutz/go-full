...
func NewFactory(fc FactoryConfig) (Factory, error) {
	if err := fc.Validate(); err != nil {
		return Factory{}, errors.New("invalid factory config: " + err.Error())
	}
...
func (f Factory) validateTime(hour time.Time) error {
	...
	if hour.After(time.Now().AddDate(0, 0, f.fc.MaxWeeksInTheFutureToSet*7)) { //nolint:mnd // magic number is from config
