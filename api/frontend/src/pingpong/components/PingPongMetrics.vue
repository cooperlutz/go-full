...
const { state, actions, totalNumberOfPingPongs, totalNumberOfPingPongsPerDay, totalNumberOfPings, totalNumberOfPongs } =
  usePingPongMetrics()
...
<template>
  ...
  <div v-else-if="state.error" id="metrics-error">Error loading metrics: {{ state.error }}</div>

  <SectionDivider />

  <!-- Line Chart -->
  <div class="card w-full bg-base-100 shadow-md card-border border-secondary border-solid">
