...
// Package httpserver is adapted from
...
// NewPromMiddleware returns a Prometheus Middleware.
func NewPromMiddleware(registry prometheus.Registerer, buckets []float64) IMiddleware {
	if buckets == nil {
		buckets = prometheus.ExponentialBuckets(0.1, 1.5, 5) //nolint: mnd // reasonable default
	}
